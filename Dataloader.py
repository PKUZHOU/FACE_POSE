...
class Multilable_Dateset(data.Dataset):
    def __init__(self,root = 'multi_angle.txt',train = True,transform = None):
        self.root = root
        self.data = {}
        self.transform = transform
        self.train = train
    ...
    def get_lists(self):
        ...
        with open(self.root) as f:
            ...
            for line in annos:
                ...
                file_path = line[0]
                yaw,roll,pitch = float(line[1]),float(line[2]),float(line[3])
                self.data[file_path] = [yaw,roll,pitch]
        num = 0
